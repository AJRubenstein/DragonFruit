...
/**
 * The slice of SupportState auto-bracing walks while regenerating kickstands.
 *
 * Pick<> rather than a bespoke shape: these ARE SupportState's collections, so a
 * field added to one of them cannot drift out of sync here.
 */
type KickstandSource = Pick<SupportState, 'kickstands' | 'roots' | 'knots'>;

function buildKickstandSamples(kickstandState: KickstandSource): SupportSample[] {
    const supports: SupportSample[] = [];
...
export function buildAutoBracedSnapshot(snapshot: SupportState, inputSettings: AutoBracingSettings): BuildSnapshotResult {
    const kickstandBase: KickstandSource = {
    ...
    let kickstandState = kickstandBase;
    let selectedKickstandCleared: boolean | null = null;
    {
        const nextKickstands: KickstandSource['kickstands'] = {};
        const nextRoots: KickstandSource['roots'] = {};
        const nextKnots: KickstandSource['knots'] = {};
        let removedAutoGeneratedCount = 0;

        for (const [id, kickstand] of Object.entries(kickstandState.kickstands)) {
            if (kickstand.generatedBy === 'autoBracing') {
        ...
        if (removedAutoGeneratedCount > 0) {
            ...
            // selection dangling.
            if (selectedKickstandCleared === null && snapshot.selectedId
                && snapshot.kickstands[snapshot.selectedId] && !nextKickstands[snapshot.selectedId]) {
                selectedKickstandCleared = true;
            }
    ...
    if (generatedKickstands.length > 0 && bracedAxesByTrunkId.size > 0) {
        ...
        if (drops.size > 0) {
            ...
            const keptKickstands: KickstandSource['kickstands'] = {};
            const keptRoots: KickstandSource['roots'] = {};
            const keptKnots: KickstandSource['knots'] = {};
    ...
    const snapshotWithKickstands: SupportState = {
        ...
        selectedId: selectedKickstandCleared ? null : nextSnapshot.selectedId,
    };
    for (const kickstand of Object.values(kickstandState.kickstands)) {
        const root = kickstandState.roots[kickstand.rootId];
        if (root) snapshotWithKickstands.roots[root.id] = root;
        const hostKnot = kickstandState.knots[kickstand.hostKnotId];
        if (hostKnot) snapshotWithKickstands.knots[hostKnot.id] = hostKnot;
    }
