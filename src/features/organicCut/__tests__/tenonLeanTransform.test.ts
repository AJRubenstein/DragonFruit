...
test('no lean and no roll is no matrix at all', () => {
  assert.equal(tenonLeanMatrix(frameAtOrigin(), 0, Math.PI / 2, 0), null);
});

test('a roll alone spins the tenon about its axis and moves nothing else', () => {
  const m = tenonLeanMatrix(frameAtOrigin(), 0, Math.PI / 2, Math.PI / 2);
...
test('the leaned axis still passes through the anchor', () => {
  ...
  for (const deg of [10, 30, 55, -35]) {
    const m = tenonLeanMatrix(frameAtOrigin(), (deg * Math.PI) / 180, Math.PI / 2, 0.4);
...
test('the lean tips the tenon in one plane, and its sign picks the side', () => {
  ...
  const plus = moved(tenonLeanMatrix(frame, 0.5, Math.PI / 2, 0)!, 0, 0, -frame.depth);
  const minus = moved(tenonLeanMatrix(frame, -0.5, Math.PI / 2, 0)!, 0, 0, -frame.depth);
