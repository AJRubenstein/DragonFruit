...
| `builtin-simple-plugin-allowlist.json` | Allowlists built-in simple plugins | **CI script** `scripts/check-plugin-boundaries.mjs` / the `generate-builtin-simple-plugins.mjs` generator |
...
- `window-layouts.json` is the known gap: it is type-cast with no validation.
