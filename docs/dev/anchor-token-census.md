...
**CORRECTED: `ANCHOR_BELOW_ROOT` is NOT the type.** This census said it was —
"raised only by the anchor's own placement rule, so it needs renaming with the
type". Reading the code disproves it: `ANCHOR_BELOW_ROOT` is a member of
...
`anchorAutoPlacement.ts` and read by `useTrunkPlacement.ts`, both sides sharing
