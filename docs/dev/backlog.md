...
| `SupportRenderer.tsx` still holds ~250 type-named identifiers, the bulk of them the eight per-type JSX render blocks (`renderTrunkList.map` ... `<TrunkRenderer trunk={...}>`). Each mounts a different component with a genuinely different prop set, so collapsing them into one loop needs a per-type component+props table -- the same hand-written list, relocated | `SupportRenderer` render blocks | **open, marked for removal** — not settled. `singular` in the registry is already the entity prop name each component takes, so the missing half is a registered renderer per type (the `registerContactBridgeBuilder` shape) plus a props contract the eight components share. Until that lands, renaming a type still means editing this file |
...
| The grep metric counts capitalised identifiers (`renderStickList`) and is blind to lowercase type-literal dispatch (`category === 'stick'`), which is the same rename hazard in a form the measurement never sees | repo-wide | **open** — 108 such sites outside `SupportTypes/`, densest in `useSupportInteractionManager` (16), `SupportSidebar` (17), `useKnotInteraction` (10), `autoPlace` (9), `autoBrace` (7). Track both numbers; a file can score well on identifiers and still name every type |
| `npm run check:lint` fails on Windows with `spawn npx ENOENT` before linting anything, so the gate reports nothing rather than failing | `scripts/check-lint-clean.mjs` | **open** — same class as `npm test` reporting 0 tests on Windows. `npx eslint --max-warnings 0 $(node -e "...directories.join(' ')")` runs the same check correctly; the script needs `shell: true` or an absolute binary path |
