...
| `autoBracing/` generates kickstands by name -- ~170 identifiers in `autoBrace.ts`, and a 587-line `generativeBracing.ts` that is placement logic for one type living outside `SupportTypes/Kickstand/` | `autoBracing/` | **open** -- plan section 8b, needs a registration slot first |
