...
| Sliding a knot along a curved kickstand shaft does not snap the result to the 0.001mm drag grid, though the other four shafted hosts do. Pre-existing, from a bulk sync commit; preserved through the collapse rather than silently changed | `useKnotInteraction` knot projection | **open** — almost certainly an oversight; one line to fix once someone confirms no kickstand path depends on unsnapped positions |
