...
| `buildScopedSupportGeometryGroup` calls six per-type group builders (`buildLeafGroup`, `buildBraceGroup`, `buildKickstandGroup` and so on) from six hand-written `payload.<collection>.forEach` blocks, one per type, and each block resolves its own inputs first: brace looks up two knots, kickstand a root plus host knot with a four-way modelId fallback, leaf its own fallback | `supportExportReconstruction.ts` | **open** — the same shape the scene's placement controllers had, but the entries are not interchangeable: each needs a preamble resolving different entities before the builder is callable. Collapsible with a `detailRenderers`-style table of `{ collection, resolve, build }`, which is a real interface design rather than a rename. Adding a type still means editing this file, as `support-type-extension.md` step 6 says |

| Kickstands live in `SupportState.kickstands` AND in a parallel `kickstandStore`, read by roughly a dozen files. Consumers that need a kickstand's roots or knots branch on `descriptor.id === 'kickstand'` to pick the store, which is the one place a type name still selects a data source | `kickstandStore.ts`, `SupportRenderer` batching loop, `supportSnapshotHelpers`, `SceneCanvas`, export and slicing paths | **open** — `support-type-extension.md` already tells new types not to copy the adapter, so the intent is to remove it, but nothing tracks the removal. Not a literal to convert: adding a registry flag for "owns its own store" would bless the duplication. Wants the store retired in favour of the collection, as its own change |
