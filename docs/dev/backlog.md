...
| Every scene snapshot clones a `kickstandState` field that NOTHING reads: no restore path, no test, no consumer anywhere. It is duplicated into undo/redo history on each transform, alongside a `supportState` that already contains the kickstands | `useSceneCollectionManager` `captureSceneSnapshot`, `page.tsx` transform history | **open** — removing the field is easy; the plumbing that feeds it runs through `page.tsx` (`kickstandBefore`/`kickstandAfter`, `captureTransformSupportSnapshot`) and wants unwinding as its own change. Started and reverted rather than leave a half-migration |
