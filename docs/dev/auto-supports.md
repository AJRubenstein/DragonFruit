...
- **Every member must clear the branch-angle rule.** `memberMaxAngleFromVerticalDeg()` derives the steepest lean a member may take from the user's branch angle (`grid.minBranchAngleDeg`, **60° above horizontal** ⇒ ≤30° from vertical) — the same rule the grid engine's `satisfiesMinAngleFromHorizontal` and the trunk-promotion path already enforce on manual branches. It bounds every auto path: the placement fan (`min(leafFanMaxAngleDeg, 30°)`), the merge knot search (`STEEP_MIN_RISE_DEG = max(45°, minBranchAngleDeg)`), the routed-branch caps, and the chunk-consolidation relaxation below. A pillar whose only neighbour is level enough to link only with a near-level bar now stays standalone instead of growing one — measured on a 30° face: 3 members over 44° and a 47° worst case become 0 over 44°, 29° worst, at the cost of 5 more standalone pillars (117 → 112). Before this the fan used 45°, merge 45° rise and consolidation 75° from vertical, and nothing consulted the configured branch angle at all.
