...
- **One branch per contact point.** Flat voxel islands (everything the grid phase does not own: `source !== 'overhang'`) get a 2.5 mm grid of branches stamped over their footprint by the legacy overhang-coverage pass. The pass walks *trunks*, so several trunks near one island would each stamp the same cell — one tentacle per trunk to the same contact ("the kraken") — and the knot ids (`auto-overhang-<trunk>-<island>-<r>-<c>`) collided when they omitted the trunk, re-parenting the whole fan onto whichever trunk wrote last. `claimedCoverageCells` now claims each cell once, the id carries the trunk, and a branch that would pierce the mesh (`branchCollidesWithSDF`) is skipped like every other member-creating path instead of being flagged `blocked` and kept.
